...
class A2CAgent(a2c_common.ContinuousA2CBase):
    ...
    def calc_gradients(self, input_dict):
        ...
        with torch.cuda.amp.autocast(enabled=self.mixed_precision):
            batch_dict['freeze_actor'] = ft_value  # NOTE this freezes the policy weights (assumes separate actor_critic)
            ...
            if self.has_value_loss:
                if ft_value:
                    val_set_split = 0.2
                    dataset_size = value_preds_batch.shape[0]
                    idx = int(dataset_size * val_set_split)
                    c_loss = common_losses.critic_loss(value_preds_batch[idx:], values[idx:], curr_e_clip, return_batch[idx:], self.clip_value)
                    with torch.no_grad():
                        validation_loss = common_losses.critic_loss(value_preds_batch[:idx], values[:idx], curr_e_clip, return_batch[:idx], self.clip_value).mean()
                else:
        ...
        if self.truncate_grads:
            if self.multi_gpu:
                ...
                self.scaler.unscale_(self.optimizer)
                nn.utils.clip_grad_norm_(self.model.parameters(), self.grad_norm)
                # print(self.model.state_dict()['a2c_network.actor_mlp.0.weight'][0,0].item())
                # print(self.running_mean_std.running_mean[10].item())
                # print(self.value_mean_std.running_mean[0].item())
                self.scaler.step(self.optimizer)
                # print(self.model.state_dict()['a2c_network.actor_mlp.0.weight'][0,0].item())
                # print(self.running_mean_std.running_mean[10].item())
                # print(self.value_mean_std.running_mean[0].item())
        ...
        self.train_result = (a_loss, c_loss, entropy, \
            ...
            mu.detach(), sigma.detach(), b_loss, validation_loss)
