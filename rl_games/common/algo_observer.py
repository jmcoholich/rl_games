...
class DefaultAlgoObserver(AlgoObserver):
    ...
    def after_print_stats(self, frame, epoch_num, total_time):
        if self.game_scores.current_size > 0 and self.writer is not None:
            ...
            wandb.log({'scores/mean': mean_scores, "frame": frame})
            wandb.log({'scores/iter': mean_scores, "epoch_num": epoch_num})
            wandb.log({'scores/time': mean_scores, "total_time": total_time})
