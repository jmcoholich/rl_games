...
def actor_loss(old_action_log_probs_batch, action_log_probs, advantage, is_ppo, curr_e_clip):
    # this is so dumb, but I discovered the what is called "action_log_probs"
    # here is actually NEGATIVE action_log_probs, hence why the calculated
    # terms should be minimized.
    if is_ppo:
        ratio = torch.exp(old_action_log_probs_batch - action_log_probs)
        surr1 = advantage * ratio
        surr2 = advantage * torch.clamp(ratio, 1.0 - curr_e_clip,
                                1.0 + curr_e_clip)
        a_loss = torch.max(-surr1, -surr2)  # then another negative here!!
    else:
