...
class A2CBase:
    ...
    def init_tensors(self):
        ...
        self.bootstraps = torch.ones((batch_size,), dtype=torch.uint8, device=self.ppo_device)
    ...
    def discount_values(self, fdones, last_extrinsic_values, mb_fdones, mb_extrinsic_values, mb_rewards, last_boostrap, mb_bootstraps):  # TODO pass bootstrap to this fn
        """ This function computes advantages
        It has been modified to only bootstrap returns upon terminations
        when the bootstrap buffer is True.

        self.tau is equal to lambda in GAE
        self.gamma is the discount factor

        mb = minibatch
        f_dones is the done value for the final step, not recorded in mb_fdones

        nextnonterminal is 1.0 is the next step is not terminal
        """
        lastgaelam = 0
        ...
        for t in reversed(range(self.steps_num)):
            if t == self.steps_num - 1:
                ...
                nextbootstrap = last_boostrap
            else:
                nextnonterminal = 1.0 - mb_fdones[t+1]
                nextvalues = mb_extrinsic_values[t+1]
                nextbootstrap = mb_bootstraps[t+1]
            nextnonterminal = nextnonterminal.unsqueeze(1)
            nextbootstrap = nextbootstrap.unsqueeze(1)

            # delta = r_t + gamma * V(s_{t + 1}) * not done - V(s_{t})
            # So here, if its terminal they don't add the next value, they never bootstrap
            # But they always bootstrap the end of the samples (due to truncation)
            # BUT if boostrap is true, then I still add nextnonterminal

            ## ORIGINAL
            delta = mb_rewards[t] + self.gamma * nextvalues * nextnonterminal - mb_extrinsic_values[t]

            ## NEW
            # delta = mb_rewards[t] + self.gamma * nextvalues * torch.logical_or(nextnonterminal, nextbootstrap).float() - mb_extrinsic_values[t]

            # Bootstrapping doesn't change the next equation at all. nextnonterminal needs to be
            # unchanged to avoid discounting rewards through different episodes
    ...
    def play_steps(self):
        ...
        for n in range(self.steps_num):
            ...
            self.experience_buffer.update_data('bootstraps', n, self.bootstraps)
            for k in update_list:
                self.experience_buffer.update_data(k, n, res_dict[k])
            if self.has_central_value:
                self.experience_buffer.update_data('states', n, self.obs['states'])

            self.obs, rewards, self.dones, self.bootstraps = self.env_step(res_dict['actions'])
            ...
            # process_infos is in rlg_train.py in isaacgym. It is only used
            # for counting consecutive successes for some environments
        ...
        last_boostrap = self.bootstraps.float()
        fdones = self.dones.float()
        mb_fdones = self.experience_buffer.tensor_dict['dones'].float()
        mb_values = self.experience_buffer.tensor_dict['values']
        mb_rewards = self.experience_buffer.tensor_dict['rewards']
        mb_bootstraps = self.experience_buffer.tensor_dict['bootstraps'].float()
        mb_advs = self.discount_values(fdones, last_values, mb_fdones, mb_values, mb_rewards, last_boostrap, mb_bootstraps)
...
class DiscreteA2CBase(A2CBase):
    ...
    def train(self):
        ...
        while True:
            ...
            log_now = epoch_num % self.wandb_log_interval == 0
            ...
            if self.rank == 0:
                ...
                if log_now:
                    wandb_log_dict = {
                        ...
                        'performance/play_time': play_time,
                ...
                if self.has_soft_aug and log_now:
                    wandb_log_dict.update({'losses/aug_loss': np.mean(aug_losses)})
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    if log_now:
                        for i in range(self.value_size):
                            assert self.value_size == 1, "Not sure in what scenarios this would be false"
                            rewards_name = 'rewards' if i == 0 else 'rewards{0}'.format(i)
                            wandb_log_dict.update({rewards_name: mean_rewards[i]})

                        wandb_log_dict.update({'episode_lengths': mean_lengths})
...
class ContinuousA2CBase(A2CBase):
    def __init__(self, base_name, config):
        ...
        self.wandb_log_interval = config.get('wandb_log_interval', 10)
    ...
    def train_epoch(self):
        ...
        if (self.game_rewards.current_size > 0
                and mean_rewards[0] > (self.best_score + self.config['score_save_interval'])):
            self.best_score = mean_rewards[0]
            print(f"=> Achieved new high score of {self.best_score:0.2f} on epoch {self.epoch_num}")

            # also save some metadata
            with open(os.path.join(self.network_path, 'metadata.txt'), 'w') as f:
                f.write(f"=> Achieved new high score of {self.best_score:0.2f} on epoch {self.epoch_num}")
            # self.save(self.network_path + self.config['name'] + '_BEST')
            self.save(self.network_path + self.config['name'])
        ...
        for _ in range(0, self.mini_epochs_num):
            ep_kls = []
            for i in range(len(self.dataset)):
                a_loss, c_loss, entropy, kl, last_lr, lr_mul, cmu, csigma, b_loss, val_loss = self.train_actor_critic(self.dataset[i])
                if self.config.get("finetune_value", False):
                    print(f"Critic Loss: {c_loss:.5e}")
                    wandb.log({
                        "a_loss": a_loss,
                        "c_loss": c_loss,
                        "entropy": entropy,
    ...
    def train(self):
        ...
        while True:
            ...
            if self.config.get("finetune_value", False):
                print("Finished value function fine-tuning")
                self.save(self.network_path + self.config['name'])
                import sys; sys.exit()
            ...
            if self.rank == 0:
                ...
                if len(b_losses) > 0 and log_now:
                    wandb_log_dict.update({'losses/bounds_loss': torch_ext.mean_list(b_losses).item()})
                if log_now:
                    wandb_log_dict.update({'losses/entropy': torch_ext.mean_list(entropies).item()})
                    wandb_log_dict.update({'info/last_lr': last_lr * lr_mul})
                    # wandb_log_dict.update({'info/lr_mul': lr_mul})
                    # wandb_log_dict.update({'info/e_clip': self.e_clip * lr_mul})
                    wandb_log_dict.update({'info/kl': torch_ext.mean_list(kls).item()})
                    wandb_log_dict.update({'epochs': epoch_num})
                    wandb_log_dict.update({'total_time': total_time})
                if self.has_soft_aug and log_now:
                    wandb_log_dict.update({'losses/aug_loss': np.mean(aug_losses)})
                ...
                if self.game_rewards.current_size > 0:
                    ...
                    if log_now:
                        for i in range(self.value_size):
                            assert self.value_size == 1, "Don't know why this would be false"
                            rewards_name = 'rewards' if i == 0 else 'rewards{0}'.format(i)
                            wandb_log_dict.update({rewards_name: mean_rewards[i]})
                            # wandb_log_dict.update({'rewards var': self.game_rewards.get_var()})

                        wandb_log_dict.update({'episode_lengths': mean_lengths})
                        # wandb_log_dict.update({'episode_lengths var': self.game_lengths.get_var()})
                if log_now:
                    wandb.log(wandb_log_dict)
                    ...
                    # if mean_rewards[0] >= self.config['score_to_win']:
                    #     self.save(self.network_path + self.config['name'])
                    #     print('Network won!')
                    #     return self.last_mean_rewards, epoch_num
                    # try:
                    #     if mean_rewards[0] > (self.best_score + self.config['score_save_interval']):
                    #         self.best_score = mean_rewards[0]
                    #         # self.save(self.network_path + self.config['name'] + '_BEST')
                    #         self.save(self.network_path + self.config['name'])
